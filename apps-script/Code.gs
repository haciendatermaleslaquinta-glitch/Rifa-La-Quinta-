...
function doGet () {
  try {
    ...
    return json_({ config: config, ticketsStatus: ticketsStatus })
  } catch (error) {
    console.error(error)
...
function doPost (event) {
  ...
  try {
    ...
    validateRequest_(request, config.ticketValue)
    ...
    if (!ticketRow || ticketRow[table.statusIndex] !== STATUS.AVAILABLE) {
      recordIncident_('Boleta no disponible', request, ticketNumber)
    ...
    file = saveReceipt_(folder, request.receipt, ticketNumber, request.name)
    ...
    setColumn_(output, table.headers, ['comprador', 'buyer'], cleanText_(request.name))
    setColumn_(output, table.headers, ['celular', 'phone', 'telefono'], cleanText_(request.phoneNumber))
    ...
    setColumn_(output, table.headers, ['vendedor', 'seller'], cleanText_(request.seller || ''))
...
function validateRequest_ (request, ticketValue) {
  ...
  if (!ALLOWED_MIME_TYPES[request.receipt.type]) throw publicError_('El formato del comprobante no está permitido.')
  const extension = String(request.receipt.name || '').split('.').pop().toLowerCase()
  if (extension !== ALLOWED_MIME_TYPES[request.receipt.type]) throw publicError_('La extensión del comprobante no coincide con su formato.')
  ...
  cleanText_(request.name)
  cleanPhone_(request.phoneNumber)
}
...
function saveReceipt_ (folder, receipt, ticketNumber, name) {
  ...
  const extension = ALLOWED_MIME_TYPES[receipt.type]
...
function readTicketTable_ (sheet) {
  ...
  const numberIndex = findHeader_(headers, ['boleta', 'ticket', 'numero'])
  const statusIndex = findHeader_(headers, ['estado', 'status'])
  if (numberIndex < 0 || statusIndex < 0) throw publicError_('La estructura de la hoja Bilhetes no es válida.')
...
function getSheet_ (name) { return SpreadsheetApp.getActive().getSheetByName(name) || getFirstSheet_([name]) }
function getFirstSheet_ (names) { const spreadsheet = SpreadsheetApp.getActive(); for (const name of names) { const sheet = spreadsheet.getSheetByName(name); if (sheet) return sheet } throw publicError_('No se encontró la hoja requerida.') }
