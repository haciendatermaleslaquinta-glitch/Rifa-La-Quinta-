...
assert.ok(document.querySelector('.payment-confirmation'), 'la confirmacion debe tener una fila propia')
assert.equal(document.querySelectorAll('.form-actions > button').length, 2, 'las acciones deben compartir una fila')
assert.equal(document.querySelector('.form-actions .button-secondary').textContent.trim(), 'Cancelar')
assert.match(document.querySelector('.form-actions .button-primary').textContent, /Enviar comprobante y registrar mi boleta/)
assert.equal(document.querySelector('.payment-confirmation').nextElementSibling.className, 'form-actions')
